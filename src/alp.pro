...
% after inserting a call or several calls to 'checkpoint(1)'
% ... 'checkpoint(n)' wherever appropriate
...
NOT ACTIVE -------------------------------------------------------------------------------------------------
for efficiency reasons, an 'expand' pass is sometimes added, covering clause level expansions at the highest
level, i.e. in finite clauses; thereby priority is given to clausal expansions at a lower level
note that if the whole string is covered, expand does not look for any clause-level expansions
-----------------------------------------------------------------------------------------------------------
...
% REMMED -------------------------------------------------------------------------------------
% runs :- run(expand).   % clausal expansions at the highest level, that of the finite clause
% --------------------------------------------------------------------------------------------
...
run(Pass) :- Pass \= output, Pass \= finite, % Pass \= expand,
...
%  'italiam peto videndae urbis in montibus positae.'  needs 5 'finite' runs to come up with the correct parse
...
/* ------------------------------------------------------
run(expand) :- [expand,_] ---> Condition_Action,
             testexec(Condition_Action),
             run(expand2).

run(expand2) :- [expand,_] ---> Condition_Action,
             testexec(Condition_Action),
             fail.
...
lorsque l'infinitif qui les complte est lui-mme passif."
...
lexarg(sequi_1,
 ...
 /* % SOLERE
lexarg(solere,
       arglist:[ws(soleo_be_accustomed,intr,clause:[],mwuw:0,
               args:[subject:[type:np,oblig:yes,constraints:[sem:[hum]]],
                     object:[type:pred,oblig:yes,constraints:[type:nonfinite_i]]])]).
*/
...
% OBLIUISCI
% Non obliviscar sermones tuos - Pascal, Mmorial
...
lexarg(uitare,
       arglist:[ws(uito_avoid,tr_cod,clause:[],mwuw:0,
                args:[subject:[type:np,oblig:yes,constraints:[hum]],
                      object:[type:np,oblig:yes,constraints:[case:acc]]])]).
% UOCARE
...
full_expansions([ clause_level_adjunct_1:[type:adjunct,    constraints:[value:purpose]], % videndae urbis in montibus positae, pacis petendae, etc.
                  ...
                  clause_level_adjunct_5:[type:advp,       constraints:[type:clausal,sem:or([eval,time,place])]],   %  graviter
...
% remmed because very expensive because rarely decidable
% i.e. most expansions end up at both sub and main clause levels

partial_expansions([ clause_level_adjunct:[type:pp,
                                           oblig:no,
			                  constraints:[prep:or([ante,apud,causa,contra,cum,de,ex,inter,intra,
					                         ob,per,praeter,pro,propter,sine,sub,trans])]],

                     clause_level_adjunct:[type:pp,      oblig:no, constraints:[prep:in, case:abl]],
                     clause_level_adjunct:[type:adjunct, oblig:no, constraints:[value:time]],
                     clause_level_adjunct:[type:adjunct, oblig:no, constraints:[value:place]],
                     clause_level_adjunct:[type:adjunct, oblig:no, constraints:[value:purpose]],
                     clause_level_adjunct:[type:adjunct, oblig:no, constraints:[value:manner_means]],
                     clause_level_adjunct:[type:advp,    oblig:no, constraints:[type:vpbound]]    ]).
...
% The very first time we meet it here, it is used to retrieve feature values for the features 'type', 'lex' and 'sem'
% such features are supposed to be part of the lexical entry for the adverb, which appears here as the FS variable
% (feature structure).
...
% the string at a certain position (between A and B we are looking for an adverb - note that A and B are free variables at
% this stage - it is the call to 'mapped' that is going to instantiate them and yield (if it is available!) the feature bundle
% associated with the adverb. Note that 'adv', the part of speech, is the name of the record 'box' or 'folder' where we store
% information pertaining to adverbs.
...
% comparative adj with comp_cplt introduced by quam - note how the presence of QUAM slows down the parsing process
% Quam is very ambiguous, and can also be a coordinator (tam ... quam)
...
[mapped(adjp,Fadjp),                                    % the future participle IS registered as an adjective
 constraint([pathlist:PL1,hp:HL1,distance:[Distnp1],
            number:Nb,gender:G,case:C,lex:Clex,type:ppf], Fadjp),
...
[mapped(flags,active(coord)),  % coordination is computationally VERY expensive,
 ...
 map(adjp,[cat:adjp,pathlist:Sorted,hp:HL,distance:[Distance],
             ...
             c_str:[head:Headinfo1,coord:Fcoord,head:Headinfo2]])].
...
ifthen(mapped(prep,[from:_,to:A|_]),(C=acc;C=gen;C=abl)),             % a simple way of enhancing efficiency : nominative and dative are barred from pps
                                                                      % and we don't have dangling preps in Latin the wy we do in English
...
% Pacis petendae oratores ad consulem miserunt (T.L) / Germanicus Aegyptum proficicitur cognoscendae antiquitatis (Tacitus)
...
ifthen(mapped(prep,[from:X,to:A|_]),(Case=acc;Case=gen;Case=abl)),                        % for efficiency resons - see above
 ...
 ifthen(Distance=1, relaxedadjacent1_cgn(Padj,[p(A,B)],Case,Gender,Nb)),
  ...
  map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[Distance],
            cat:np,class:Class,sem:Sem,
            number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
            case:Case,w:Weight,c_str:[head:FSnoun,adjp:FSadj]])].
...
% participle clause (mostly following the noun they get attached to)
%
...
% ifthen(Length > 1, Extpp > Extnp),          % the participle phrase, if longer than one word, follows the noun ???????
...
% NOT CLEAR WHETHER WE NEED THE FOLLOWING RULE
/*
[finite,np2ab] --->
[mapped(np,FSnp),
 mapped(adjp,FSadj),
 constraint([pathlist:PLnp,hp:HPnp,index:Index,distance:Distance,w:Weightnp,
             ...
             number:Nb,gender:Gender,case:Case,class:Class,
             sem:Sem,lex:LexNoun],FSnp),
 constraint([number:Nb,gender:Gender,case:Case,type:Type,lex:_,w:W],FSadj),
 (Type=ppr;Type=ppf),				% 'shortened' participial clauses
                                                                        % not all of them active here
									% ppr and gdiv are left out
									% and have to be registered as common-or-garden-type adjectives
 constraint([pathlist:PLpp],FSadj),
 append(PLnp,PLpp,PLbignp),
 extremity(PLpp,Extpp),
 length(PLpp,Length),
 extremity(PLnp,Extnp),
 ifthen(Length > 1, Extpp > Extnp),                             % the adj phrase, if longer than one word, follows the noun
 relaxadjacent(PLnp,PLpp),                      % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
 msort(PLbignp, Sorted),
 \+dup(Sorted),
 % Weight is W+Weightnp+1,
  myplus(W,Weightnp,Weight),
 % myplus(T1,1,Weight),
 map(np,[pathlist:Sorted,hp:HPnp,index:Index,distance:Distance,cat:np,class:Class,sem:Sem,
          number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
          case:Case,w:Weight,c_str:[head:FSnp,adjp:FSadj]])].
...
[mapped(flags,active(coord)),     % making sure we have a coordinator
 ...
 mapped(np,Fnp2),
 mapped(coord,[from:X,to:Y|Fcoord]),
 ifthenelse(constraint([lex:quam],Fcoord), WeightCoord=10,WeightCoord=0),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% code below to be revised: rel clause precedes antecedent ; antecedent has hic... is... ipse... idem.... 
...
% let's pay tribute :
%	dca			       choisis  qui dire          : toi seule me plais
...
[ mapped(relative_clause,FS2),
  constraint([number:Nb,gender:G,case:Case_in_Rel,pathlist:PL2,distance:[Distrel],
...
[mapped(flags,active(coord)),  % we have a coordinator somewhere in the string
 ...
 map(pp,[pathlist:Sorted, hp:HL, distance:[Distance],index:_,
           ...
           w:Weight,
           c_str:[head:Head1,coord:Fcoord, head:Head2]])].
...
% we copy voice mood and tense from the first verb
  ...
  map(vgpos,[cat:vg,class:Class, voice:Voice,type:Type,pathlist:[p(A,D)],hp:[p(A,B),p(C,D)],
              ...
              c_str:[head:Fverb1,coord:Fcoord,head:Fverb2]])].
...
% adverb inside vg positive polarity only

[verb, vgadv] --->
[ mapped(vgpos,FSVG),
  mapped(advp,FSADV),
  constraint([pathlist:PADV, type:vpbound, lex:LexAdv],FSADV),
  constraint([pathlist:PV],FSVG),
  adjacent(PADV,PV),
  append(PADV,PV,Path),
  pick(pathlist:PV, FSVG,FSVG1),
  append([pathlist:Path,adverb:LexAdv],FSVG1,FSVGN),
  map(vg,FSVGN)].
...
% the I_down and I_up indices are used to allow percolation of the subjects
% in the main clause (up) and in the subordinate (down) (as I have just said... bis repetita)
...
% the weight is meant to reflect the nber and nature of the arguments satisfied - the more the better
 ...
 % the belonging of adjuncts is too often undecidable
 % so that too many solutions are generated
 % we need a sophisticated pragmatic discourse-oriented module to deal adequately with this overgeneration
  ...
  insort(AllTrees, ST),                                          % standard order imposed on args IN THE PARSE TREE
   ...
   clause_constraints(Clause_Constraints,FSverb),   % they concern polarity (so far...) 
						...
						% prioritize normal subject-object order (following a suggestion made by Dominique Longre, ULg)
...
/*
"agere negotium procurandi fructus mortis":
...
[ mapped(p_p,[from:B,to:C|FSpp]),
  ...
  map(pred,[cat:pred,type:aa,case:abl,mood:aa,
               number:sing,person:3,
              pathlist:Path_aa, distance:[0],
              gap:[],w:MW,c_str:[lex:Lex,pp:FSpp,gdiv:FSgdiv]])].
...
map(pred,[cat:pred,type:finite,pathlist:PLtot,distance:Distance,
            ...
            number:N,person:2, mood:subjunctive,
...
mapped(pred,FSpred),
constraint([type:finite,pathlist:PL,distance:Distance,number:N,person:1,
            mood:subjunctive,
            tense:present,
            argbound:no,
...
mapped(pred,FSpred),
constraint([type:finite,pathlist:PL,distance:Distance,number:N,person:Person,
            mood:subjunctive,
            tense:Tense,
            argbound:no,
            polarity:Pol,
...
map(pred,[cat:pred,type:finite,pathlist:PLtot,distance:Distance,
            ...
            number:N,person:Person, mood:subjunctive,
 ...
 full_expansions(FE),
  ...
  flatten(CPathlist,CFlat),
   ...
   ifthenelse(mapped(punct,[from:Lastbutone,to:Fin,lex:question_mark|_]),
                ifthenelse(member(p(Lastbutone,Fin),APF),Int=yes,(append([p(Lastbutone,Fin)],APF,AP), Int=yes)),
              AP=APF),
...
*/
 ...
 % myplus(T2,MW,Wtot),
  ...
  map(pred,[cat:pred,
              ...
              number:Nsubj,			% in FINITE clauses, number and person refer to nber and person
						% of the subject
              person:Psubj,
 ...
 illoc_force(Int,statement) :-
            var(Int).

 illoc_force(Int,question) :-
            nonvar(Int).
...
The reason being that even finite clauses are building blocks, and should be expanded BEFORE they click into place in the
finite clause that is going to house them; there is no way of isolating a process that should be applied once all others have
had a chance to apply - lastima, we have to pursue our way - no Brooklyn for us.


[expand,last_exit_to_brooklyn] --->

[    mapped(pred,FSpred),
     constraint([type:finite,pathlist:PLpred, distance:DistPred,tense:Tense,
                 polarity:Pol,argbound:no,mood:Mood,gap:[],w:Wpred,flagint:IntFlag,c_str:[VG|ST]],FSpred),

     mapped(fin, fin(Fin)),           % we have to know where the string ends
     not(member(expanded:yes,FSpred)), % we do not attempt to re-expand an expanded clause


     ifthen(path(0,Fin,PLpred), fail), % the path must not cover all the words of the sentence already !

      full_expansions(FE),

     expandkernel(FE,ExpandTrees,ExpandPaths,ExpandDistances,w:WExpand, Int,_),

      ifthenelse(var(Int),
            (IntFlag=no,Force=statement),             % statement if Int is still a free var at this stage
            (IntFlag=wh_question, Force=question)),   % check if we have a wh-question


     myplus(WExpand,Wpred,Wtot),

     append(ExpandPaths,PLpred,Paths),
     flatten(Paths,Flat),
     msort(Flat,FlatSorted),
     \+ dup(FlatSorted),


      myappend(DistPred,ExpandDistances,Distances),
      flatten(Distances, FlatDist),
      sum(FlatDist, Distance),

      append(ST,ExpandTrees,AllTrees),
      insort(AllTrees, NST),
      ...
      map(pred,[cat:pred,
             ...
             flagint:IntFlag,
              c_str:[VG|NST]])                 % NST includes the trees garnered in the expansion process
...
[ mapped(part,[from:A,to:B|Fint]),
 ...
 %  checkpoint(1),
 %  ifthenelse(mapped(punct,[from:Lastbutone,to:Fin,lex:question_mark|_]),
  %            append([p(Lastbutone,Fin)],PL1,AP),
   %           AP=PL1),
...
[ mapped(sub,[from:A,to:B|Fsubordinator]),
 ...
 % start(PL1,B), % too strict

 append([p(A,B)],PL1,PL2),
 msort(PL2,Sorted),
 ifthenelse( recorded(stg,[texttype,2]),quasicontiguous(Sorted), contiguous(Sorted)),     % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%

 map(free_sub,[type:finite, pathlist:PL2, distance:[Distp1],gap:[],subordinator:LexSub,
...
[ mapped(sub,[from:A,to:B|Fsubordinator]),
 mapped(pred,Fp1),
 mapped(pred,Fp2),
 Fp1 \= Fp2,				% subordinator + 2 clauses

 constraint([argbound:no],Fsubordinator),   % the subordinator is not necessarily arg-bound,
					    % subordinators that can occur at the head of free subordinate clauses
					    % are supposed to bear the no value for the argbound feature

 ifthenelse( mapped(punct,[from:X,to:Y, lex:comma|_]),
             Adtopath=[p(X,Y)],
             Adtopath=[]),						% comma ?
 % subordinate clause

 constraint([type:finite,
             pathlist:PL1,distance:[Distp1], gap:[],argbound:no,
             mood:MoodSub,
             tense:TenseSub,
             w:W1,c_str:Head1],Fp1),

start(PL1,B),						% subordinate clause starts at end of subordinator NOT NECESSARILY SO

% main clause

 constraint([type:finite,
             pathlist:PL2,distance:[Distp2], gap:[],argbound:no,
             mood:MoodMain,
             tense:TenseMain,
             w:W2,c_str:Head2],Fp2),

							% after the comma if any

 ifthen(Adtopath=[p(X,Y)],           % if there is a comma,
            extremity(PL1,X)),		 % the comma sits in between the 2 preds
                       % if no comma, end of subordinate clause is start of main clause

extremity(PL1,EndSub),
EndSub1 is EndSub+1,
(start(PL2,EndSub); start(PL2,EndSub1)),

 append([p(A,B)], PL1,NPL1),                     % add subordinator to subordinate clause
 msort(NPL1,NPL1sorted),

 % quasicontiguous(NPL1sorted),			 % room for one word to escape       % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%

 quasicontiguous(NPL1sorted),

 append(NPL1sorted,Adtopath,PL1new),             % add comma to the path if necessary (Adtopath is set accordingly)
 append(PL1new,PL2,PL),
 msort(PL, Sorted),
  \+dup(Sorted),
...
[finite,predfsuba] --->
[ mapped(sub,[from:Y,to:Z|Fsubordinator]),        % subordinator starts just after end of main clause
 mapped(pred,Fp1),
 mapped(pred,Fp2),
 Fp1 \= Fp2,
 ...
 ifthenelse( mapped(punct,[from:X,to:Y, lex:comma|_]), Adtopath=[p(X,Y)], Adtopath=[]), % comma ?

 % main clause
 constraint([type:finite,pathlist:PL1,distance:[Distp1], gap:[],argbound:no,
             mood:MoodMain, tense:TenseMain,w:W1,c_str:Head1],Fp1),
 start(PL1,0),
 % subordinate clause
 constraint([type:finite,pathlist:PL2,distance:[Distp2], gap:[],argbound:no,
             mood:MoodSub, tense:TenseSub,w:W2,c_str:Head2],Fp2),

 ifthen(Adtopath=[p(X,Y)],
            extremity(PL1,X)),		 % the comma sits in between the 2 preds
                      % extremity of main clause is start of subordinator


 start(PL2,Z),	                               % subordinate clause starts at end of subordinator
 append([p(Y,Z)], PL2,NPL2),                      % subordinator joined to subordinate clause
 msort(NPL2,NPL2sorted),
 quasicontiguous(NPL2sorted),
 append(PL1,Adtopath,PL1new),			% add comma to the path if there is one
 append(PL1new,NPL2sorted,PL),
 msort(PL, Sorted),
  \+dup(Sorted),
 contiguous(Sorted),                              % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
 % Distance is Distp1+Distp2,
 % Weight is W1+W2,
  myplus(Distp1,Distp2,Distance),
   myplus(W1,W2,Weight),
 ifthen( (constraint([lex:si],Fsubordinator),
          MoodMain=indicative,
          MoodSub=indicative),
          Clausesem=open),
 ifthen( (constraint([lex:si],Fsubordinator),
 ...
 ifthen( (constraint([lex:si],Fsubordinator),
          ...
          TenseMain=pluperfect),
          Clausesem=irrealis_past),
 ifthen( (constraint([lex:cum],Fsubordinator),		       % CUM for time full stop
          MoodSub=indicative,
          MoodMain=indicative),
          Clausesem=modality_free_time_clause),
 ifthen( (constraint([lex:cum],Fsubordinator),			% CUM for time+
          MoodSub=subjunctive,
          MoodMain=indicative),
          Clausesem=modality_laden_time_clause),
 map(pred,[cat:pred,type:finite,pathlist:Sorted,distance:[Distance], gap:[],
             mood:MoodMain, tense:TenseMain,w:Weight, argbound:no,
             c_str:[clause_semantics:Clausesem,main_clause:Head1, subordinator:Fsubordinator,subordinate_clause:Head2]])
].
...
% with ***subject*** in front of both clauses:
% rex, cum amasset servas, legisset libellos impudicos
%

% here the comma is likely to prove essential to guide the parsing !

[finite,predfsub] --->
[mapped(sub,[from:A,to:B|Fsubordinator]),
 constraint([argbound:no],Fsubordinator),

 mapped(np,FSnp),				% the part played by the element in front of the two clauses
						% is generally that of subject, although other basic syntactic roles
						% are not excluded
						% here the restriction to subject is imposed
						% therefore to be revised !!!!!!!!!!!!!!!!!!

 constraint([number:Nsubj,                      % all are subject values !
             person:Psubj,
             gender:_,
             case:nom,
             pathlist:Pathlistnp,
             w:WNP,
             distance:[Distnp]],FSnp),
 extremity(Pathlistnp,AA),

 ifthenelse( mapped(punct,[from:AA,to:A, lex:comma|_]), Adtopath1=[p(AA,A)], Adtopath1=[]), % comma ?

 append(Pathlistnp,Adtopath1,Pathlistnpfull),

 mapped(pred,Fp1),
 mapped(pred,Fp2),
 Fp1 \= Fp2,

 ifthenelse( (mapped(punct,[from:X,to:Y, lex:comma|_]), X\=AA), % another comma ?
             Adtopath=[p(X,Y)],
             Adtopath=[]),
 % subordinate clause
 constraint([type:finite_i,pathlist:PL1,distance:[Distp1], gap:[],
             number:Nsubj,person:Psubj,
             mood:MoodSub, tense:TenseSub,w:W1,c_str:Head1],Fp1),
 start(PL1,B),								% subordinate clause starts at end of subordinator

% main clause
 constraint([type:finite_i,pathlist:PL2,distance:[Distp2], gap:[],
             number:Nsubj,person:Psubj,
             mood:MoodMain, tense:TenseMain,w:W2,c_str:Head2],Fp2),
 start(PL2,Y),				 % after the comma if any

 ifthenelse(Adtopath=[p(X,Y)],           % if there is a comma,
            extremity(PL1,X),		 % the comma sits in between the 2 preds
            extremity(PL1,Y)),           % if no comma, end of subordinate clause is start of main clause
 append([p(A,B)], PL1,NPL1),                     % add subordinator to subordinate clause
 msort(NPL1,NPL1sorted),
 quasicontiguous(NPL1sorted),
 append(NPL1sorted,Adtopath,PL1new),             % add comma to the path if necessary (Adtopath is set accordingly)
 append(PL1new,PL2,PL),
 append(Pathlistnpfull,PL,PLfull),
 msort(PLfull, Sorted),
  \+dup(Sorted),
 quasicontiguous(Sorted),                       % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
 % Distance is Distp1+Distp2+Distnp,
 % Weight is W1+W2+WNP,
  myplus(Distp1,Distp2,TDis1),
  myplus(TDis1,Distnp,Distance),
   myplus(W1,W2,T1),
   myplus(T1,WNP,Weight),
 ifthen( (constraint([lex:si],Fsubordinator),			% OPEN condition
          MoodMain=indicative,
          MoodSub=indicative),
          Clausesem=open),
 ...
 map(pred,[cat:pred,type:finite,
             pathlist:Sorted,distance:[Distance],
             gap:[],
             mood:MoodMain, tense:TenseMain,
             w:Weight,
             argbound:no,
             c_str:[clause_semantics:Clausesem,
                    subject:FSnp,subordinator:Fsubordinator,
                    subordinate_clause:Head1,
                    main_clause:Head2]])
].
...
[mapped(punct,[from:StP1,to:StartSubordinator, lex:comma|_]), % in front of Subordinator
 ...
 mapped(pred,MainClause),
 mapped(pred,SubClause),

 MainClause \= SubClause,

 % subordinate clause
 constraint([type:finite,pathlist:PLSub,distance:[Distp1], gap:[],
             number:Nsubj,person:Psubj,
 ...
 % main clause
 constraint([type:finite,pathlist:PLMain,distance:[Distp2], gap:[],
             number:NMain,person:PMain,
             mood:MoodMain, tense:TenseMain,w:W2,c_str:HeadMain],MainClause),
 ...
 append(PLMain,NPL1sorted,PLfull),
 msort(PLfull, Sorted),
  \+dup(Sorted),
 contiguous(Sorted),                       % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
 % Distance is Distp1+Distp2,
 ...
 map(pred,[cat:pred,type:finite,
             ...
             argbound:no,
             c_str:[clause_semantics:Clausesem,
                    subordinator:Fsubordinator,
                    subordinate_clause:SubClause,
...
% similar treatment for the other functions -
% they can be bundled together if we are ready to have
% a variable in the position of feature name :
% "Function_in_relative_clause:Function, Function:[e:Index]"
% The procedure is all right if the feature name gets instantiated before it is used
% otherwise the shit is likely to hit the fan
% caution asks for a more restricted use of uninstantiated variables
...
[finite,rel_clause_2] --->
[mapped(relative,[from:X,to:Y|FS1]),
 constraint([number:Nb, case:Case,gender:Gender, function:Functions],FS1),
 mapped(pred,FS2),
 constraint([type:finite,mood:Mood, tense:Tense,pathlist:Pathlist,distance:Distance,w:Weight],FS2),
 msort(Pathlist,Sorted),
 start(Sorted,Y),
 contiguous(Sorted),                                      % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
...
% indirect object masquerading as direct object by displaying accusative case
%
...
[mapped(prep,[from:A, to:B|FS1]),		% we have a prep
 ...
 mapped(pred,FS3),
 constraint([type:finite, mood:Mood, tense:Tense,pathlist:Pathlist,distance:Distance,w:Weight],FS3),
 msort(Pathlist,Sorted),
 start(Sorted,C),
 contiguous(Sorted),                    % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%%
...
[ifthenelse( mapped(punct,[from:AA,to:A, lex:comma|_]), % comma ?
 ...
 mapped(pred,FS3),
 constraint([type:finite, mood:Mood, tense:Tense,pathlist:Pathlist,
...
[ifthenelse( mapped(punct,[from:AA,to:A, lex:comma|_]), % comma ?
 ...
 mapped(pred,FS3),
 constraint([type:finite, mood:Mood, tense:Tense,
...
%
 ...
 % the other types of conjuncts concern the clause itself and are dealt with
 % elsewhere,
...
% in quasicontiguous we allow one or two or even three elements to be out of place, i.e.
% out of the clause they belong to

quasicontiguous(L) :- contiguous(L), !. % Qui peut le plus...

quasicontiguous(L) :- pick(_,L,L1), contiguous(L1), !.

quasicontiguous(L) :- pick(A,L,L1), pick(B,L1,L2),contiguous([A,B]),contiguous(L2), !.
...
% formerly USED in SE-reference binding
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% EITHER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% assignable_index(Index,Position) is used to check that the index popped from the Indexlist is ok with respect
						% to the position of SE in string

% index is i(p(Start,End)) and position is [(p(Start,End)]

assignable_index(i(p(Stindex,Endindex)),[p(StPos,EndPos)]) :- (Endindex =< StPos ; EndPos=Stindex).

% 0 rex 1 putat 2 se 3 reginam 4 amare 5
% index assigned to rex		is 0,1 ok with respect to position of se, which is 2,3 because 1(End of rex) =<  2 (start of se)
% index assigned to reginam	is 3,4							because 3(End of se) = 3 (Start of reginam)




% selectright(Index,IndexList,Position,NewIndexList) is used to select an index for SE that has been assigned
% to an np of the right kind that +- PRECEDES the occurrence of SE

selectright(Index,[Index|IndexList],Position,IndexList) :-
    assignable_index(Index,Position), !.

selectright(Index, [H|IndexList], Position, [H|NewIndexList]) :-
    selectright(Index,IndexList,Position,NewIndexList).

selectright(Index,[Index],Position,[]).                       %  only one index available, we assign it
selectright(nil,[],Pos,[]).
...
selecttop2([],Position,nil,nil) :- !.           % no candidate : we assign nil twice as indices for the SE
selecttop2([Index],Position,Index,nil) :- !.    % one candidate only : we assign it as first candidate, and nil as second

selecttop2(IndexList,[p(SP,EP)],First,Second) :-  % two or more candidates in the IndexList
      PositionCount is SP+EP,				% PositionCount is the sum of the borders of SE in the clause: start position + end position
      Boundary is EP+2,					% boundary that cannot be crossed - the referent of SE should immediately follow
							% or, more naturally, precede SE in the string
      assign_counts(IndexList,CountedList,PositionCount,Boundary),  % we compute proximity of the indices to the string position of SE
      sort(CountedList,[C1-First,C2-Second|T]).                     % we sort them and select the best, i.e. the nearest to SE


assign_counts([H|T],[HC|TC],PositionCount,Boundary) :-
                                                  assign_count(H,HC,PositionCount,Boundary),
						  assign_counts(T,TC,PositionCount,Boundary).
assign_counts([],[],_,_).

assign_count(i(p(S,E)), C1-i(p(S,E)), PositionCount,Boundary) :-
				IC is S + E,							% sum of the borders for the candidate index
                                ifthenelse(S < Boundary,                                        % it should be left of SE, or just behind
				                      (C is PositionCount - IC,                 % C reflects nearness of the Index to SE - work it out if you feel doubtful
                                                       ifthenelse(C < 0, C1 is -C, C1 is C)),   % turning negative C values into positives, so that they do not
												% cluster in front of the others

                                           C1 is 100). % a value that puts it out of the competition, to be increased if necessary ;-



USE EITHER

 " selectright(I1,IndexList,Position,IndexList1),
 selectright(I2,IndexList1,Position,IndexList2) "

OR

" selecttop2(IndexList,Position,I1,I2)"

as in:


ifthen(member(index:IndexSubj,FSSpecs),IndexCurr=IndexSubj),
ifthen(constraint([lex:Lex],FSSpecs),   % if the arg has a lexical head
                (ifthenelse( Lex=pp3refl, % IF
				(   constraint([hp:Position,index:i(I1,I2)],FSSpecs),
				    recorded(ilist,IndexList,_),
				    selecttop2(IndexList,Position,I1,I2) ),  % THEN


                                (   ifthen(constraint([sem:[hum]],FSSpecs),
                                             (recorded(ilist,IndexList,Ref),                      % ELSE

                                                 ifthen( (nonvar(IndexCurr), \+member(IndexCurr,IndexList)), % IF
                                                       (push(IndexCurr,IndexList,NewIndexList),
                                                        erase(Ref),
                                                        recorda(ilist,NewIndexList,_)
                                                        )
                                              )
                                 )
                             )
...
% the following lines of code are too strict if we attempt to deal with multiple-clause sentences
% we would need to specify clause limits before we can use such a process
% notice the NOT - we do not have a subject in the string
% but we are not looking for one as we would in gapped structures

/*
not((mapped(T,FSSpecs),
     constraint([number:Nsubj,person:Psubj,case:nom,index:Index],FSSpecs),
...
% similar treatment all through - they can all be bundled up together if we use a variable for the function name
% but it seems to entail duplications and a huge computational cost ! To be investigated.
...
match(object:Specs,
        object:OutSpecs,
        [number:Nsubj,gender:Gendersubj,person:_],
        PathlistArg,
        DistanceArg,
        ClauseType, % finite/nonfinite distinction is irrelevant for non-subject args - NOT QUITE projection of subject on object SE in finite clauses only
        gap:[],
        w:W,
        Int) :-

% extracting info from the Specs, i.e. what we need
constraint([type:T],Specs),
constraint([constraints:Cs],Specs),

% checking that we have it
mapped(T,FSSpecs),
ifthen(constraint([type:int],FSSpecs),Int=int),  % keeping track of args consisting of, or containing, an interrogative
...
match(agent:Specs,agent:OutSpecs,[number:Nsubj,gender:Gendersubj,person:_],
...
ifthenelse(constraint([lex:Lex],Out),   % if the arg has a lexical head
                                ifthenelse( (Lex=pp3refl,ClauseType=finite),                     % and it is SE
												% we can project subject nber and gender on the SE
				   (
                                  constraint([number:NumberinSpecs,gender:GenderinSpecs],Out), % retrieving gender and number from the proposed subject
                                  funify([number:Nsubj,gender:Gendersubj],[number:NumberinSpecs,gender:GenderinSpecs],[number:NNsubj,gender:GGsubj]),
...
% standard indirect objects
%

match(i_object:Specs,i_object:OutSpecs,[number:Nsubj,gender:Gendersubj,person:_],
        PathlistArg,DistanceArg,ClauseType,gap:[],w:W, Int) :-
constraint([type:T],Specs),
constraint([constraints:Cs],Specs),
mapped(T,FSSpecs),
...
% indirect objects in the accusative case
%

% in double-accusative constructions (doceo-type verbs)

% they are registered as object_i args

match(object_i:Specs,object_i:OutSpecs,[number:Nsubj,gender:Gendersubj,person:_],
        PathlistArg,DistanceArg,ClauseType,gap:[],w:W, Int) :-
constraint([type:T],Specs),
constraint([constraints:Cs],Specs),
mapped(T,FSSpecs),
...
ifthenelse(constraint([lex:Lex],Out),   % if the arg has a lexical head
                                ifthenelse( (Lex=pp3refl,ClauseType=finite),                    % and it is SE and the clause is finite
												% we can project subject nber and gender on the SE
				   (
                                  constraint([c_str:[head:[number:NumberinSpecs,gender:GenderinSpecs]]],Out), % retrieving gender and number from the proposed subject
                                  funify([number:Nsubj,gender:Gendersubj],[number:NumberinSpecs,gender:GenderinSpecs],[number:NNsubj,gender:GGsubj]),
...
% COMPLEMENT (unspecified)
%

% this arg type is used when we do not really care to further specify the nature of the argument
% used with specific predicates such as some of the impersonal

match(cplt:Specs,cplt:OutSpecs,[number:Nsubj,gender:Gendersubj,person:_],
       PathlistArg,DistanceArg,ClauseType,gap:[],w:W, Int) :-
constraint([type:T],Specs),
constraint([constraints:Cs],Specs),
mapped(T,FSSpecs),
...
match(adjunct:Specs,adjunct:OutSpecs,[number:_,gender:_,person:_],
       PathlistArg,DistanceArg,_,gap:[],w:W, Int) :-
...
/*
full_expansions([ clause_level_adjunct_1:[type:adjunct,constraints:[value:purpose]], % videndae urbis in montibus positae, pacis petendae, etc.
                  clause_level_adjunct_2:[type:adjunct,    constraints:[value:time]],      % Romae, tres annos, ...
                  clause_level_adjunct_3:[type:adjunct,    constraints:[value:place]],
                  clause_level_adjunct_4:[type:adjunct,    constraints:[value:manner_means]],
                  clause_level_adjunct_5:[type:advp,       constraints:[]],      % grauiter
                  clause_level_adjunct_6:[type:advp,       constraints:[type:or([clausal,int])]],  % etiam

                  clause_level_adjunct_7:[type:pp,constraints:[prep:or([ante,apud,causa,circum,contra,cum,de,ex,inter,intra,
				                              ob,per,praeter,pro,propter,sine,sub,trans])]],         % cum magna voce
                  ...
                  clause_level_adjunct_8:[type:pp,         constraints:[prep:in, case:abl]],  % in horto, in ecclesia tua
                  dativus_ethicus:[type:np,constraints:[case:dat,lex:or([pp1sg,pp2sg,pp3refl,pp1pl,pp2pl])]],
                  ablative_absolute:[   type:pred,       constraints:[type:aa]]    ]). % rege sub tegmine fagi recubante
...
% the not-recorded condition on the parse tree tries to avoid duplicates

% OUTPUT MODE 1
%

% the parses are checked and printed one by one;
% the recorda/recorded pair avoids duplication
% the rerunning of the 'output' pass run ensures that all relevant parses are produced

% such an output procedure cannot be used if a sorting of the parses according to weight is required
% that's the reason why this mode is presently remmed

% in order for it to be usable the single pass setting condition on the output run should be removed
% it is included in the metainterpreter - see (far...) above

/*
[output,results] --->
[ mapped(pred,Pred),
 constraint([cat:pred, type:finite,pathlist:Pathlist,distance:[Distance],
             gap:[],w:Weight,c_str:Parse],Pred),
   \+dup(Pathlist),
  Rating is Weight-Distance,
  not(recorded(parse,parse(Distance-Parse),_)),
  mapped(fin,fin(Fin)),
  once(path(0,Fin,Pathlist)),      % once specifies a single call,
                                   % not involved in backtracking
  recorded(out,Lists,_),
  nl,nl(Lists),
  write('Rating -> '), write(Rating), nl,
  write(Lists,'Rating -> '), write(Lists,Rating), nl(Lists),
  once(prpr(Parse,0,user)),
  once(prpr(Parse,0,Lists)),
  nl(Lists),
 ...
 % writeq(Lists,Pred), % writing raw parses for debugging purposes if needed
  nl,nl(Lists),
 recorda(parse,parse(Distance-Parse),_)].
...
% alternate way of outputting results
%

% this output mode is to be preferred because it involves sorting of the parses according to their ratings
...
[recorded(out,Lists,_),
 ...
 setof(Rating-Parse,
             Pred^Pathlist^Distance^Weight^Fin^(mapped(pred,Pred),   % existential quantification to avoid multiple results
                                                ...
                                                % constraint([cat:pred, type:finite,argbound:no,pathlist:Pathlist,distance:[Distance],
                                                  %     gap:[],w:Weight,c_str:Parse],Pred),
...
/*
% Printing the parses needs to take the Rating into account
% this is ensured by prpr2

% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% we print a maximum of 3 parses per sentence
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

% no parse
printthem([],Stream) :- nl(Stream), !.

% three parses or more
printthem([H1,H2,H3|_],Stream) :- nl(Stream),
                                  prpr2(H1,0,Stream),
                                  nl(Stream), nl(Stream),
                                  prpr2(H2,0,Stream),
                                  nl(Stream), nl(Stream),
...
% two parses
printthem([H1,H2],Stream) :- nl(Stream),
                                  prpr2(H1,0,Stream),
                                  nl(Stream), nl(Stream),
                                  prpr2(H2,0,Stream), !.
...
%
% Detach -QUE and -CUM where appropriate - + deals with nec (--> et non)
%
     ...
     deque([H|T],[H|T1]) :- deque(T,T1).
               % the word does not end in -que, neither is it food for the other processes described here
